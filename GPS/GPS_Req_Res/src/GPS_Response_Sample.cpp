...
class Response_Sample
{
    private:
        ...
        //blocked must be initialized before the thread is started 
    ...
    public:
        ...
        Response_Sample(bool _use_static_routing):
        app_(vsomeip::runtime::get()->create_application()),
        is_registered_(false),
        use_static_routing_(_use_static_routing),
        blocked_(false),
        running_(true),
        offer_thread_(std::bind(&Response_Sample::run,this))
        {
        ...
        bool init()
        {
            ...
            app_->register_state_handler(
                std::bind(&Response_Sample::on_state,this,std::placeholders::_1)
            );

            app_->register_message_handler(
                SERVICE_ID,INSTANCE_ID,METHOD_ID,
                std::bind(&Response_Sample::on_message,this,std::placeholders::_1)
            );
...
#ifndef VSOMEIP_ENABLE_SIGNAL_HANDLING
  Response_Sample *rs_ptr(nullptr);

  void handle_signal(int _signal)
  {
        if(rs_ptr != nullptr && (_signal==SIGINT || _signal == SIGTERM))
        rs_ptr->stop();
...
int main(int argc,char **argv)
{
    ...
    Response_Sample rs(use_static_routing);
